...
Picture
\end{center} 
The output of a one pebble transducer is the concatenation of the outputs of the transitions it took, in the order of their emission.
...
The initial configuration is $(q_I, 0)$. The final configuration is $(q_F,|w|+1)$ The automaton accepts $w$ if there is an accepting run,
...
%Similarly, the sequence   $(T_j)_{j\in[i+1,k]}$ can be seen as a $(k-i)$-pebble transducer, of input alphabet $\Sigma$ and output alphabet $\Gamma \cup [1,i]$. We denote them respectively by $\Tt[1,i]$ and $\Tt[i+1,k]$.
 ...
 \medskip
\begin{terminology} 
 A $k$-pebble transducer $\Tt$ can be seen as the one-pebble transducer $T_k$, which outsources a part of the computation to the other one-pebble transducers $T_i$, $i<k$. A $T_i$ can then itself call as subroutines the transducers of lower indexes, and so on.
...
We present in this section a tool used to summarize the behaviour of one-pebble automata, and called its transition monoid (resp. morphism). We map each word  $w$ to an element of the monoid which gives the folliwing kind of information: if the automaton enters the word from the left in state $q$, then it exits to the \textit{e.g}~left in state $q'$, \textit{etc}.
...
We define the transition monoind $M_N$ of $T$ as follows:
\begin{itemize}
...
\end{itemize}
\end{definition}
We will mainly instanciate $M_N$ in the following three cases: 
\begin{enumerate}
